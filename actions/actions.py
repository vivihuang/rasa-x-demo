...
class AllenNlpAction(Action):

    def __init__(self):
        self.answer = ""

    def name(self) -> Text:
        return "action_get_allen_res"

    def _get_res(self, question: str) -> str:
        if question is not None:
            from actions.allen_res import get_allen_res

            logger.debug("Getting res from website.")
            self.answer = get_allen_res(question)
    ...
    def run(
        ...
        answer = self._get_res(question=question)

        if answer is "":
            dispatcher.utter_template("utter_no_allen_res", tracker)
        else:
